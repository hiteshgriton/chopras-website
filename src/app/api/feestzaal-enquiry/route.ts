...
const FROM_ADDRESS = 'Chopras Feestzaal <noreply@chopras.nl>'
...
function buildEmailHtml(fields: {
  ...
  const rows = [
    ...
    ...(fields.message ? [{ label: 'Message / Special Requests', value: fields.message }] : []),
  ]
        ...
        <table width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#fff;border-radius:12px;overflow:hidden;box-shadow:0 2px 12px rgba(27,43,94,0.1);">
          ...
          <tr>
            <td style="background:#1B2B5E;padding:28px 32px;">
              <p style="margin:0;font-family:sans-serif;font-size:11px;font-weight:700;color:#C7A348;text-transform:uppercase;letter-spacing:0.18em;">New Enquiry</p>
              <h1 style="margin:6px 0 0;font-family:serif;font-size:26px;color:#fff;font-weight:600;">Feestzaal Huren / Event Booking</h1>
              <p style="margin:6px 0 0;font-family:sans-serif;font-size:12px;color:rgba(255,255,255,0.55);">Submitted via ${formLabel} &bull; Chopras Indian Restaurant Den Haag</p>
            </td>
          </tr>
          ...
          <tr>
            <td style="padding:20px 32px 32px;">
              <p style="margin:0 0 12px;font-family:sans-serif;font-size:13px;color:#1A1A1A/70;">Reply directly to this email to respond to ${fields.name}. Their address is <strong>${fields.email}</strong>.</p>
              <a href="mailto:${fields.email}?subject=Re: Feestzaal enquiry from ${encodeURIComponent(fields.name)}"
...
export async function POST(request: NextRequest) {
  try {
    ...
    const html = buildEmailHtml({ name, email, phone, date, guests, eventType, message, source })

    const { error } = await resend.emails.send({
      from: FROM_ADDRESS,
      to: TO_ADDRESSES,
      replyTo: email,
      subject: `Feestzaal Enquiry: ${eventType} — ${name} (${guests})`,
      html,
    })
    ...
    console.error('Feestzaal enquiry route error:', err)
    return Response.json(
